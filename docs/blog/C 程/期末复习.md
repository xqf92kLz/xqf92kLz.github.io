??? info "查漏补缺"
    ...
    - 预处理指令
        ...
        `#include "文件名"`：一般预处理器将在当前源程序所在的目录中寻找该文件。如果在当前目录中没有找到该文件，那么编译器将在标准目录（某些特定的目录，由编译器实现厂商决定）中寻找该文件

        `#include<文件名>`：预处理器将在标准目录中寻找这些文件
    ...
    - 字符串：
        ...
        `char *p="hello";strcpy(p,"world");` 会 RE，因为 "hello" 是字符串常量，p 指向的是只读内存
    ...
    - typedef
        ...
        typedef 与 define 区别：比如 `typedef int* IntPtr;`（记得不要漏掉 `;`）和 `#define IntPtr int*`，后者定义 `IntPtr a,b;` 会变成 `int* a,b;`（b 是 int），前者则 a 和 b 都是 int*
...
??? warning "注意"
    ...
    sizeof 包含字符串末尾的 '\0'，strlen 不包含 '\0'。注意看清楚 sizeof(s) 的 s 是字符串还是字符数组。注意 sizeof(数组名) 算的不是数组大小，是 数组大小 × 类型的字节数
