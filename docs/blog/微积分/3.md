...
- 拉格朗日中值定理：见到“$f-f$”或者“$f$ 与 $f'$ 关系”时
	...
	??? example "例题"
		...
		??? note "解答"
			即证 $\dfrac 1{1+x}<\dfrac{\ln(1+x)-\ln(1+0)}{x-0}<1$。
			
			令 $f(t)=\ln(1+t)$，则 $f(t)$ 在 $[0,x]$ 上连续，在 $(0,x)$ 内可导。根据拉格朗日中值定理，$\exists \xi\in(0,x)$ 使得 $\dfrac{\ln(1+x)}x=\dfrac 11 {1+\xi}$。
			
			而 $\dfrac 1 {1+x}<\dfrac 1 {1+\xi}<1$，故 $\dfrac 1 {1+x}<\dfrac{\ln(1+x)}x<1$。
		...
		??? note "解答"
			任务 $x\in \mathring U_+(x_0)$，则 $f(x)$ 在 $[x_0,x]$ 上连续，在 $(x_0,x)$ 内可导，根据拉格朗日中值定理，$\exists \xi\in(x_0,x)$ 使得 $\dfrac{f(x)-f(x_0)}{x-x_0}=f'(\xi)$。当 $x\to x_0+0$ 时，$\xi\to x_0+0$，因此 $f_+'(x_0)=\lim\limits_{x\to x_0+0}\dfrac{f(x)-f(x_0)}{x-x_0}=\lim\limits_{x\to x_0+0}f'(\xi)=\lim\limits_{x\to x_0+0}f'(x)$。
