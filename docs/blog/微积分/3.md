...
- 麦克劳林（Maclaurin）公式：$x_0=0$ 时的泰勒公式。
	...
	$f(x)=\sum_{k=0}^n\frac{f^{(k)}(0)}{k!}x^k+\frac{f^{(n+1)}(\xi)}{(n+1)!}x^{n+1}$，$\xi$ 介于 $0$ 和 $x$ 之间
...
- 函数的凹凸性
	...
	- 定义：
		...
		1. 设 $f(x)$ 在区间 $D$ 上连续，若对 $\forall x_1,x_2\in D$ 均有 $f(\frac{x_1+x_2}2)\geq \frac{f(x_1)+f(x_2)}2$，则称 $y=f(x)$ 在区间 $D$ 上是凸的。同样可定义凹的。
