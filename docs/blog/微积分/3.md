...
- $\frac{*}{\infty}$ 型
	...
	??? example "例题"
		计算：（1）$\lim\limits_{x\to \infty}\dfrac{x+\sin x}{x-\sin x}$；（2）$\lim\limits_{x\to 0}\dfrac{x^2\sin\frac 1 x}{e^x-1}$。

        ??? note "解答"
            （1）本题为 $\dfrac{\infty}{\infty}$ 型极限，若用洛必达法则，有 $I_1=\lim\limits_{x\to \infty}\dfrac{1+\cos x}{1-\cos x}$，但后者极限并不存在，因此不能用洛必达法则。

            （2）本题为 $\dfrac 0 0$ 型极限，若用洛必达法则，有 $I_2=\lim\limits_{x\to 0}\dfrac{2x\sin \frac 1 x-\cos\frac 1 x}{e^x}$，此极限不存在，因此不能用洛必达法则。
            
            ???+ warning "注意"
				使用洛必达法则时，应注意：所求极限为 $\frac 0 0$ 或 $\frac * {\infty}$ 型；分子分母的导数存在；导数比 $\frac{f'(x)}{g'(x)}$ 的极限存在。
