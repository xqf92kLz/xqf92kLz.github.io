    ...
    - 数列与子数列收敛性的关系
        ...
        ???+ info "拓展"
        	...
        	??? note "证明"
        		...
        		数列 $\{a_n\}$ 存在单调递增或递减的子列 $\{a_{n_k}\}$，又数列 $\{a_n\}$ 有解，从而子列 $\{a_{n_k}\}$ 单调且有界。根据单调收敛准则，子列 $\{a_{n_k}\}$ 收敛。
...
- 若 $\{y_n\}$ 从某一项开始严格递增且 $\lim\limits_{n\to \infty}b_n=+\infty$，且 $\lim\limits_{n\to \infty}\frac{x_n-x_{n-1}}{y_n-y_{n-1}}$ 存在或为 $\pm \infty$，则 $\lim\limits_{n\to \infty}\frac{x_n}{y_n}=\lim\limits_{n\to \infty}\frac{x_n-x_{n-1}}{y_n-y_{n-1}}$。

	也就是说，分式形式的数列，若分母数列严格递增，且差分后的极限存在，则原式必有极限。
	
	几何意义：把 $(x_n,y_n)$ 看作平面上的点 $M_n$，定义意义是，假设 $M_n$ 的横坐标 $x_n\to +\infty$，那么当 $\vec{M_nM_{n+1}}$ 的斜率以 $a$ 为极限时，则 $\vec{OM_n}$ 的斜率也以 $a$ 为极限。
...
- 
	??? example "例题"
        ...
        令 $f_n(x)=x^n+nx-2\,(x>0)$，根记为 $x_n$。求 $\lim\limits_{n\to \infty}(1+x_n)^n$。
        
        ??? note "解答"
        
        	$x_n^n+nx_n-2=0$
        	
        	$f_n(0)<0$，$f_n(0)=n-1$。$n>1$ 时，$x_n\in(0,1)$，则 $x_n^n\to 0$，$nx_n\to 2$。
        	
        	$\lim\limits_{n\to \infty}(1+x_n)^n=\lim\limits_{n\to \infty}(1+\dfrac 2 n)^n=\lim\limits_{n\to \infty}(1+\dfrac 2 n)^{\frac n 2\cdot \frac {2n} n}=e^2$