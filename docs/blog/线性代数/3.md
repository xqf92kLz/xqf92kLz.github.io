...
- 矩阵的运算对秩的影响
	...
	- 转置，数乘，求逆：$r(A)=r(A^T)$，$r(cA)=\begin{cases}0,&c=0\\r(A),&c\neq 0\end{cases}$，$r(A_n^{-1})=r(A_n)=n$（$A_n$ 可逆）
	
	- 乘法：
	
		- $r(A)=r(PA)=r(AQ)=r(PAQ)$，其中 $A_{m\times n}$，$P_{m\times m}$，$Q_{n\times n}$，且 $P,Q$ 可逆
		- $\underbrace{r(A)+r(B)-s\leq r(AB)}_{\text{Sylvester 不等式}}\leq \min\{r(A),r(B)\}$，其中 $A_{m\times s},B_{s\times n}$
			...
			??? note "证明"
				- 证明 Sylvester 不等式：
					...
					则 $r(AB)+s=r\begin{pmatrix}E&O\\O&AB\end{pmatrix}=r\begin{pmatrix}E&B\\A&O\end{pmatrix}\geq r\begin{pmatrix}O&B\\A&O\end{pmatrix}=r(A)+r(B)$
		
		- Frobenius（弗罗贝尼乌斯）不等式：$r(ABC)\geq r(AB)+r(BC)-r(B)$

	- 加减法：$r(A\pm B)\leq r(A)+r(B)$
...
- 有关秩的几个重要式子
	...
	设 $A$ 是 $m\times n$ 的矩阵，$B$ 是满足有关矩阵运算要求的矩阵，则：
	
	- $AB=O\Rightarrow r(A)+r(B)\leq A$ 的列数
	...
	- $r(A^*)=\begin{cases}n,&r(A)=n\\1,&r(A)=n-1\\0,&r(A)<n-1\end{cases}\,(n\geq 2)$，其中 $A$ 为 $n$ 阶方阵
		...
		??? note "证明"
			...
			- 当 $r(A)=n-1$ 时：
				...
				$A$ 中存在 $n-1$ 阶子式不为 $0$，则 $A^*\neq O$，即 $r(A^*)\geq 1$。
				
				又 $|A|=0$，所以 $AA^*=|A|E=O$。
				
				根据矩阵秩的性质（若 $AB=O$，则 $r(A)+r(B)\leq A$ 的列数），则 $r(A)+r(A^*)\leq n\Rightarrow n-1+r(A^*)\leq n\Rightarrow r(A^*)\leq 1$。
