	...
	- 非齐次线性方程组的通解
		...
		???+ example "例题"
			...
			??? note "解答"
				...
				设 $\boldsymbol\eta_0$ 是方程 $\boldsymbol A\boldsymbol x=\boldsymbol b$ 的一个解，$\boldsymbol{\xi}_1,\boldsymbol{\xi}_2,\cdots,\boldsymbol{\xi}_{n-r}$ 为 $\boldsymbol A\boldsymbol x=\boldsymbol 0$ 的基础解系。
