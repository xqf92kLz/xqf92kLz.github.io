...
- 线性方程组解的结构（针对无穷多解）
	...
	- 非齐次线性方程组的通解
		...
		???+ example "例题"
			$\boldsymbol A_{m\times n}\boldsymbol X=\boldsymbol b$，设 $r(\boldsymbol A)=r(\boldsymbol{\overline A})=r<n$。记 $W=\{\boldsymbol X\mid \boldsymbol A\boldsymbol X=\boldsymbol b,\boldsymbol X\in\mathbb P^n\}$，问 $\boldsymbol W$ 中是否存在极大线性无关组？
