...
- 导数的概念
    ...
    - 函数可导与连续的关系：若 $f(x)$ 在 $x=x_0$ 处可导，则 $f(x)$ 在 $x=x_0$ 处必连续。即，$f'(x_0)$ 存在 $\Rightarrow$ $\lim\limits_{x\to x_0}f(x)=f(x_0)$。
    	...
    	???+ example "例题"
    		...
    		??? note "解答"
    			- 连续：$\lim\limits_{x\to 0}f(x)=f(0)=0$
    			...
    			- 导函数连续：$f'(0)$ 存在且 $\lim\limits_{x\to 0}f'(x)=f'(0)=0$
...
- 导数的基本公式与运算法则
    - 几个基本初等函数的导数
        ...
        ??? note "证明"
        	$\lim\limits_{\Delta x\to 0}\dfrac{\sin(x+\Delta x)-\sin x}{\Delta x}=\lim\limits_{\Delta x\to 0}\dfrac{2\cos\frac{2x+\Delta x}{2}\sin\frac{\Delta x}{x}}{\Delta x}=\lim\limits_{\Delta x\to 0}\dfrac{2\cdot \frac{\Delta x}{2}\cos\frac{2x+\Delta x}{2}}{\Delta x}=\cos x$
    ...
    - 导数的四则运算：设 $u=u(x),v=v(x)$ 在点 $x$ 处可导，则 $u\pm v$，$u\cdot v$，$\frac u v\,(v\neq 0)$ 都在点 $x$ 处可导，且 $(u\pm v)'=u'\pm v'$，$(uv)'=u'v+uv'$，$(\frac u v)'=\frac{u'v-uv'}{v^2}\,(v\neq 0)$。
    	...
    	???+ example "例题"
    		...
    		??? note "解答"
    			- **法 1：**$f'(0)=\lim\limits_{x\to 0}\dfrac{f(x)-f(0)}{x-0}=\lim\limits_{x\to 0}\dfrac{(e^x-1)(e^x-2)\cdots(e^x-2025)}x=\lim\limits_{x\to 0}(e^x-2)\cdots(e^x-2025)=2024!$
			
				- **法 2：**记 $g(x)=(e^x-2)(e^x-3)\cdots(e^x-2025)$，则 $f(x)=(e^x-1)g(x)$。$f'(x)=e^xg(x)+(e^x-1)g'(x)$，则 $f'(0)=g(0)=2024!$。
...
???+ warning "注意"
    ...
    - 注意 $f'(x_0)>0$ 不能说明 $f(x)$ 在 $U(x_0)$ 单调递增，只能说明 $\lim_{x\to x_0}\dfrac{f(x)-f(x_0)}{x-x_0}>0$。
    
        例如 $f(x)=\begin{cases}\frac x 2+x^2\sin\frac 1 x&(x\neq 0)\\0&(x=0)\end{cases}$，$f'(x)=\begin{cases}\frac 1 2+2x\sin\frac 1 x-\cos\frac 1 x&(x\neq 0)\\\frac 1 2&(x=0)\end{cases}$。对 $x=0$ 的任何一个邻域 $U(0,\delta)$，$\exists n\in\mathbb N$ 使得 $\dfrac 1 n<\delta$，取 $x_n=\pm\dfrac 1 {2n\pi}$，$y_n=\pm\dfrac 1 {2n\pi+pi}$，有 $f'(x_n)=-\dfrac 1 2$，$f'(y_n)=\dfrac 3 2$。因此不存在 $\delta>0$，使得 $f(x)$ 在 $U(0,\delta)$ 内单调。
    	
    	尽管 $f'(x_0)>0$，但并不存在 $\delta>0$ 使得 $f(x)$ 在 $(x_0-\delta,x_0+\delta)$ 内单调递增。
