...
- 线性替换：$\boldsymbol x=\boldsymbol C\boldsymbol y$
	...
	若 $\boldsymbol C$ 可逆，则 $\boldsymbol A$ 与 $\boldsymbol B$ 合同，称为非退化线性替换。
...
- 标准形：只有平方项、没有交叉项的二次型，$g(\boldsymbol y)=\boldsymbol y^T\boldsymbol \Lambda\boldsymbol y$。
    ...
    化标准形：任意 $\mathbb P^{n\times n}$ 中的对称矩阵必与某个 $\mathbb P^{n\times n}$ 中的对角矩阵合同。写出 $\boldsymbol A$，求出 $\boldsymbol Q^T\boldsymbol A\boldsymbol Q=\boldsymbol \Lambda$，令 $\boldsymbol x=\boldsymbol Q\boldsymbol y$。（可以用配方法。特别地，如果 $\mathbb P=\mathbb R$，还可以用实对称矩阵的正交相似对角化或成对初等变换法）
...
- $\boldsymbol A$ 正定 $\Rightarrow$ $\boldsymbol A^{-1}$，$k\boldsymbol A$（$k$ 为正实数），$\boldsymbol A^*$，$\boldsymbol A^k$（$k$ 为正整数），$\boldsymbol C^T\boldsymbol A\boldsymbol C$（$\boldsymbol C$ 实可逆）均正定
	
	（先证实实对称矩阵，然后前四个从 $\lambda>0$ 角度证明，最后一个从 $\boldsymbol A=\boldsymbol D^T\boldsymbol D$ 角度证明）

	若 $\boldsymbol A,\boldsymbol B$ 正定，则 $\boldsymbol A+\boldsymbol B$ 正定（用定义证），$\begin{pmatrix}\boldsymbol A&\boldsymbol O\\\boldsymbol O&\boldsymbol B\end{pmatrix}$ 正定（用特征值或顺序主子式或 $\boldsymbol D^T\boldsymbol D$ 或定义证）
...
- 最值问题：设 $\boldsymbol A$ 的特征值 $\lambda_1\leq \lambda_2\leq \cdots\lambda_n$，则 $\lambda_1\boldsymbol x^T\boldsymbol x\leq f(\boldsymbol x)=\boldsymbol x^T\boldsymbol A\boldsymbol x\leq \lambda_n\boldsymbol x^T\boldsymbol x$

	且若 $\boldsymbol x^T\boldsymbol x=1$，则 $f_{\min}=\lambda_1,f_{\max}=\lambda_n$