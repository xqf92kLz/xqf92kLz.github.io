...
- 段寄存器：表示段地址
	...
	cs 不能用 mov 指令赋值；ds、es、ss 可以用 mov 指令赋值，但源操作数不能是常数，只能是寄存器 ax、bx、cx、dx、sp、bp、si、di 或变量（word ptr 宽度的）
...
- 标志寄存器：存储标志位
	...
	1. 6 个状态标志：反映当前指令的执行情况，cf、zf、sf、of、pf、af
        ...
        - （第 4 位）af：加法时低 4 位向高 4 位进位、减法时低 4 位向高 4 位借位时，af = 1，否则 af = 0
        	...
        	af 没有相关的跳转指令，它与 BCD 码调整指令入 `aaa`、`aas`、`daa`、`das` 有关
...
- 常数
	...
	注意常数表达式中只能包含运算符和常数，不能含有寄存器或对变量值的引用。例如 `mov ax,bx+2`、`mov dx,(byte ptr [si])*3` 都是错误的。
	
- 标号
- 变量

	代码段中，可以用 `var` 或 `[var]` 引用变量 var，用 `a[1]`或 `[a+1]` 引用 db 类型数组 a 的第一个元素
	
	数据段中，可以用 `var` 或 `offset var` 作为伪指令 ddw 的操作数表示变量 var 的偏移地址（近指针），可以用 `var` 作为伪指令 dd 的操作数表示变量 var 的远指针。如 `addr1 dw abc` = `addr1 dw offset abc`，`addr2 dd xyz` = `addr2 dw offset xyz,seg xyz`
	
	代码段的指令中，只能用 `offset var` 引用变量 var 的偏移地址，`seg var` 或数据段名引用该变量的段地址
...
- 转换指令
	...
	- `movzx dest,src`：格式 `movzx reg16,reg8|mem8`，`movzx reg32,reg8|mem8|reg16|mem16`，将 src 零扩充到 dest
...
- 十进制调整指令
	...
	非压缩 BCD 码采用 8 个二进制位表示 1 个十进制位，其中 8 个二进制位中的高 4 位没有意义，即可以为任意值
...
- 控制转移指令
	...
	- 无条件跳转指令
		...
		- `jmp short dest` / `jmp dest`：短跳。机器码有 2 个字节，dest 与下条指令距离在 [-128,127]
			...
			机器码 `0EBh,idata8`，操作是 IP = IP（下条指令的偏移地址）+ 符号扩充为 16 位的 idata8
			
			idata8 = 目标地址 - (当前这条短跳指令的偏移地址 + 2) = 目标地址 - 下条指令的偏移地址
			
		- `jmp near ptr dest` / `jmp dest`：近跳。机器码有 3 个字节，dest 与下条指令距离在 [-32768,+32767]
		
			机器码 `0E9h,idata16_L8,idata16_H8`，操作是 IP = IP（下条指令的偏移地址）+ idata16
			
			idata16 = 目标地址 - (当前这条近跳指令的偏移地址 + 3) = 目标地址 - 下条指令的偏移地址
			
		- `jmp far ptr dest` / `jmp dest`：远跳。机器码有 5 个字节
		
			机器码 `0EAh,idata32_L16,idata32_H16`，操作是 IP = idata32_L16，CS = idata32_H16
	...
	- 子程序调用与返回指令
		...
		- `call near ptr dest` / `call dest`：近调用。机器码有 3 个字节，dest 与下条指令距离在 [-32768,+32767]
			
			机器码 `0E8h,idata16_L8,idata16_H8`，操作是 push IP（下条指令的偏移地址），IP = IP + idata16
		
			idata16 = 目标地址 - (当前指令的偏移地址 + 3) = 目标地址 - 下条指令的偏移地址
			
		- `retn` / `retn idata16`：近返回（在多数情况下可以简写为 ret）
		
			`retn` 的操作是 pop IP
			
			`retn idata16` 的操作是 pop IP, sp += idata16
			
		- `call far ptr dest`：远调用。机器码有 5 个字节
			
			机器码 `9Ah,idata32_L16,idata32_H16`，操作是 push CS, push IP, IP = idata32_L16, CS=idata32_H16
		
		- `retf` / `retf idata16`：远返回
		
			`retf` 的操作是 pop IP，pop CS
			
			`retf idata16` 的操作是 pop IP, pop CS, sp += idata16
	
	- 中断和中断返回指令
	
		- `int idata8`：调用 n 号中断
			
			机器码 `0CDh,idata8`，操作是 pushf, push CS, push IP, IF = 0，TF = 0, IP = word ptr 0000:[idata8\*4], CS = word ptr 0000:[idata8\*4+2]
		
		- `int 3`：软件断点中断
		
			机器码 `0CCh`，操作是 pushf, push CS, push IP, IF = 0, TF = 0, IP = word ptr 0000:[000Ch], CS = word ptr 0000:[000Eh]
		
		- `into`：溢出中断。如果 of == 1 则调用 4 号中断（int 4h）
		
			机器码 `0CEh`，操作是若 OF == 1，则 pushf, push CS, push IP, IF = 0, TF = 0, IP = word ptr 0000:[0010h], CS = word ptr 0000:[0012h]
			
		- `iret`：中断返回
		
			操作是 pop IP，pop CS，popf