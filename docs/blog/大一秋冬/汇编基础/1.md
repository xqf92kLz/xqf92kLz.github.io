...
- bochs 虚拟机（[下载链接](http://cc.zju.edu.cn/bhh/bochs@bw.zip)）
	...
	若调试，用 `ldr 1`。F6 或命令 `EC` 让光标在命令窗和代码窗之间切换。命令 `D 地址` 在数据窗中观察该地址指向的数据，命令 `U 地址` 在代码窗中观察该地址指向的函数体，`U cs:ip` 或 `.` 在代码窗中回到当前正在跟踪的指令处。F2 在代码窗设置/取消断点。`bpx 地址` 在该地址处设置一个软件断点。`bl` 列出已有的断点，`bd 编号` 禁用断点。
